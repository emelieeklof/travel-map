...
/**
 * The photo + info card for one spot — everything below/including the hero photo.
 * Shared by `SpotDetail` (full-page) and `CollectionMapView`'s bottom-sheet overlay,
 * which each wrap this in their own container/close affordance.
 */
export function SpotDetailContent({ placeId }: { placeId: string }) {
  ...
  const priceDisplay = priceLevelToDollarSigns(place.priceLevel)
    ...
    <>
      ...
      <div className="px-4 relative">
        <div className="py-4">
          ...
          <div className="text-sm text-on-surface-variant mt-1 flex items-center gap-1.5 flex-wrap">
            <span>{place.address ?? cat.label}</span>
            {priceDisplay && <span className="text-outline">· {priceDisplay}</span>}
            {isClosed && (
          ...
          {place.googleMapsUri && (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="inline-flex items-center gap-1 text-xs text-primary mt-1 font-medium"
            >
          ...
          {place.openingHours && place.openingHours.length > 0 && (
            <div className="mt-4 border-t border-outline-variant/50 pt-3">
              <button
                ...
                className="flex w-full items-center justify-between text-sm text-on-surface"
              >
                <span className="flex items-center gap-1.5 font-semibold">
                  <Clock size={15} className="text-on-surface-variant" /> Opening hours
                </span>
                <ChevronDown size={16} className={`text-outline transition-transform ${hoursOpen ? 'rotate-180' : ''}`} />
              </button>
              {hoursOpen && (
                <ul className="mt-2 space-y-1">
                  {place.openingHours.map((line, i) => (
                    <li key={i} className="text-xs text-on-surface-variant">{line}</li>
                  ))}
                </ul>
              )}
            </div>
